function NSTEXT:WEEKNUMOFYEAR() : NSTEXT
  -- Example of code that returns the week number according to ISO8601
  ...
  -- Main wrapper function to calculate the weeknum
  local function getWeekNumberOfYear(tm)
    ...
    if( (dayOfYearCorrected > 0) and weekNum == 53) then
     ...
     -- Add 0 prefix if weeknum is less than 10 so that it is normalzied to 2 characters
    if(weekNum.Int <= 9) then
    local weekNum = string.format("%02d", weekNum.Int)
    end 
    return weekNum
  end
  
  -- return the calculated week number to ADC
